...
function App() {
  const [produto, setProduto] = useState({cod: "", nome: '', preco: '', quantidade: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setMensagem(texto);
        setProduto({cod: "", nome: '', preco: '', quantidade: '' });
        fetchProdutos();
      }
    } catch (error) {
      setTipoMensagem('erro');
  ...
  return (
    <div className="container">
      ...
      <form onSubmit={handleSubmit}>
        <input name="cod" type="number" placeholder="Código do Produto" value={produto.cod} onChange={handleChange} required />
      ...
      <ul>
        {produtos.map((p, index) => (
          <li key={index}>{p.cod} - {p.nome} - R$ {p.preco.toFixed(2)} - Quantidade: {p.quantidade}</li>
        ))}
      </ul>
    </div>
  );
